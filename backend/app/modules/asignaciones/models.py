...
class Asignacion(Base):
    ...
    usuario_id   = Column(UNIQUEIDENTIFIER, ForeignKey("usuarios.id"),   nullable=True)
...
class HistorialAsignacion(Base):
    ...
    id            = Column(UNIQUEIDENTIFIER, primary_key=True, default=uuid.uuid4)
    asignacion_id = Column(UNIQUEIDENTIFIER, ForeignKey("asignaciones.id", ondelete="CASCADE"),
                           nullable=False)
    estado_anterior = Column(String(15), nullable=True)
    estado_nuevo    = Column(String(15), nullable=False)
    cambiado_por    = Column(UNIQUEIDENTIFIER, ForeignKey("usuarios.id"), nullable=True)
    fuente          = Column(String(50), default="sistema")
    nota            = Column(Text, nullable=True)
    created_at      = Column(DateTime, server_default=func.now())
    created_at = Column(DATETIME2, server_default=func.now(), nullable=False)
